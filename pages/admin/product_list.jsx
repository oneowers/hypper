...
export default function Example() {
    ...
    const handleUpdateProduct = (productId) => {
        fetch(process.env.REACT_APP_API_BASE_URL + `/api/product/${productId}/`, {
            method: 'PATCH',
            headers: {
...
// Update the local state without reloading the page
    ...
    const handleDelete = (item_id) => {
        ...
        fetch(process.env.REACT_APP_API_BASE_URL + `/api/product/${item_id}`, {
            method: 'DELETE',
        })
            .then((response) => {
                if (!response.ok) {
...
// Optionally, you can handle the error and provide feedback to the user
    ...
    return (
        <SkeletonTheme baseColor="#111827" highlightColor="#1E2B45">
            <div
                className="-mx-px grid grid-cols-2 sm:mx-0 md:grid-cols-3 lg:grid-cols-4"
            >
                <div
                    className="m-3 rounded-lg border-dashed border-2 border-indigo-500 py-7 text-center font-bold text-base text-indigo-500"
                    onDrop={handleDrop}
                                ...
                                <tr key={item.id}
                                    ...
                                    <td>
                                        <Link to={"/product/" + item.id}
                                            ...
                                            <img src={item.image} className="h-8 w-8 rounded-full bg-gray-800"/>
                                            <div
                                    ...
                                    <td className="hidden py-4 pl-0 pr-8 leading-6 md:table-cell lg:pr-20">
                                        {item.discount_price != 0 ? (
                                            <>
                                                <del className='text-xs text-gray-600'>{item.price} сум</del>
                                                <br/>
                                                <div className='text-sm text-gray-400'>{item.discount_price} сум</div>
                                            </>
                                        ) : (
                                            <div className='text-sm text-gray-400'>{item.price} сум</div>
                                        )}
                                    </td>
